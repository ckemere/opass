function [z,gam,ngam,muu,Phi,Nu,Kappa,S]=asugs(x,A,sig, params);
% function [z,gam,ngam,muu,Phi,Nu,Kappa,S]=asugs(x,A,sig, params);
    % Runs a modified OPASS algorithm (using the ASUGS updates)
    % x is of size NxK, A is the dictionary, and sig is the covariance matrix of x
    ...
    if isfield(params,'verbose')
        verbosity = getfield(params,'verbose');
    else
        verbosity = inf;
    end
    ...
    %%%%% sig is the covariance matrix of a 90x4 (i.e., 360x1) observation
    ...
    while curndx<N-P-rang
        ...
        if cSpike>C
            C=cSpike;
            debugprintf(2,'New class (%d). Thr: %d\n', C, peak_thr);
            if (C == Cmax)
                debugprintf(1, 'Ending early because Cmax reached.\n');
                return;
            end
        end
        ...
        S(:,Qt)=yhat;
        sz=sz+1;
        curndx=Qt+ round(0.005 * samplingrate); % step forward 1 ms
...
function debugprintf(vlevel, varargin)
    if (verbosity > vlevel)
        fprintf(varargin{:});
    end
end
